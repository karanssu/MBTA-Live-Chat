...
router.post("/", async (req, res) => {
    ...
    if (!friendUser)
        return res.status(404).send({ message: "FriendUserId not found." });

    // Check if already Friends
    const areFriends = await friendModel.findOne({
        username: user.username,
        friendUsername: friendUser.username,
    });
    if (areFriends)
        return res.status(200).send({ message: "Users are already Besties." });
